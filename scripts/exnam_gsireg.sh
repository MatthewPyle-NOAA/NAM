...
#                      .                                             .
# Script name:         exnam_gsireg.sh.ecf
# Script description:  Runs regional GSI variational analysis for NAM parent
#
...
#Check to see if ensembles were found ; if so copy filelist03 to COMOUT
if [ ! -s filelist${nhr_assimilation} ]; then
  echo "Ensembles not found - turning off HYBENS!"
  HYB_ENS=".false."
else
  mv filelist${nhr_assimilation} $COMOUT/${RUN}.${cycle}.filelist${nhr_assimilation}.location.${tmmark}
  # make filelist03 to be read into code
  ls ens_* >>filelist${nhr_assimilation}
fi

# Turn off use of GFS O3 for FV3GFS
USEGFSO3=.false.
...
useref_envir=$GESDIR/nam.t${cyc}z.useref_envir_parent_${tmmark}.sh
...
#Save a copy of the reject lists in ${RUN}.hold for future use in case some lists are missing
#  Note that the same list is used in the nest - so we only need o copy over one set.
#  As such the parent handles this and this is therefore skipped in the nest GSI step

cp *_rejectlist ${gespath}/${RUN}.hold/.
...
then
  ...
  then
    ...
    then
      ...
      echo "WARNING : NO SATBIAS_PC FILE AVAILABLE FOR CYCLE $CYCLE!"
    fi
  fi
fi
...
if [ $tmmark = "tm06" ]
then
  cpreq $GESDIR/nam.t${cyc}z.input_domain_01_nemsio.tm06 ./wrf_inout${nhr_assimilation}
fi
...
then
  if [ $tmmark = tm00 ] ; then
    if [ $GUESStm00 = NAM ] ; then
      cpreq $GESDIR/nam.t${cyc}z.nmm_b_restart_nemsio.${tmmark_prev} ./wrf_inout${nhr_assimilation}
    fi
    if [ $GUESStm00 = GDAS ] ; then
      cpreq $GESDIR/nam.t${cyc}z.input_domain_01_nemsio.tm00 ./wrf_inout${nhr_assimilation}
    fi
  else
    cpreq $GESDIR/nam.t${cyc}z.nmm_b_restart_nemsio.${tmmark_prev} ./wrf_inout${nhr_assimilation}
  fi
...
then
  cpreq $GESDIR/nam.t${cyc}z.nmm_b_restart_nemsio.ges ./wrf_inout${nhr_assimilation}
fi
...
if [ $GUESS = GDAS -a $tmmark = tm06 ] ; then
   mv wrf_inout${nhr_assimilation} $GESDIR/${RUN}.t${cyc}z.input_domain_01_nemsio.anl.${tmmark}
   export err=$?;err_chk
else
   if [ $RUNTYPE = CATCHUP -a $tmmark = tm00 ] ; then
     if [ $GUESStm00 = NAM ] ; then
        mv wrf_inout${nhr_assimilation} $GESDIR/${RUN}.t${cyc}z.nmm_b_restart_nemsio_anl.${tmmark}
     fi
     if [ $GUESStm00 = GDAS ] ; then
        mv wrf_inout${nhr_assimilation} $GESDIR/${RUN}.t${cyc}z.input_domain_01_nemsio.anl.${tmmark}
     fi
   ...
   then
     if [ $tmmark = tm00 ] ; then
       if [ $GUESStm00 = NAM ] ; then
         ...
         cpreq $GESDIR/${RUN}.t${cyc}z.nmm_b_restart_nemsio_anl.${tmmark} ${gespath}/${RUN}.hold/nmm_b_restart_nemsio_hold.${cyc}z
       else
         echo "NAM run off GDAS 1st guess, no restart file"
       fi
     fi
   fi
fi
...
if [ $tmmark != tm00 ] ; then 
  cp satbias_out ${gespath}/nam.hold/satbias_in
  cp satbias_out ${gespath}/nam.hold/satbias_in.${cya}.${tmmark}
fi

cp satbias_pc.out $COMOUT/${RUN}.t${cyc}z.satbias_pc.${tmmark}

if [ $tmmark != tm00 ] ; then 
  cp satbias_pc.out ${gespath}/nam.hold/satbias_pc
  cp satbias_pc.out ${gespath}/nam.hold/satbias_pc.${cya}.${tmmark}
fi
...
if [ $tmmark != tm00 ] ; then 
  cp $RADSTAT ${gespath}/${RUN}.hold/radstat.nam
fi
