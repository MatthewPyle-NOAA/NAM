...
!
  ...
  startmsg
  ${MPIEXEC} -n ${procs} -ppn ${procspernode} $EXECnam/emcsfc_gridgen_sfc >> $pgmout 2>errfile
  export err=$?;err_chk
  ...
  startmsg
  ${MPIEXEC} -n ${procs} -ppn ${procspernode} $EXECnam/nam_geogrid >> $pgmout 2>errfile
  export err=$?;err_chk
...
if [ $CYCNEST = true ]; then
  ...
  cpreq ncepdate.npstm06 $DATA/metgrid/.
...
if [ $CYCNEST = true ]; then
  ...
  cp $PARMnam/nam_Vtable.NAM Vtable

  hrs="00"

  for hr in ${hrs}; do

  cpreq $COMIN/nam.t${cyc}z.awpges01.tm01.grib2 awpges01

  done

  ln -sf awpges01 GRIBFILE.AAA
...
startmsg
${MPIEXEC} -n ${procs} -ppn ${procspernode} $EXECnam/nam_ungrib >> $pgmout 2>errfile
export err=$?;err_chk
...
cpreq $PARMnam/nam_METGRID.TBL.NMM_noncycnests METGRID.TBL
#end CYCNEST test
fi
...
startmsg
${MPIEXEC} -n ${procs} -ppn ${procspernode} $EXECnam/nam_metgrid >> $pgmout 2>errfile
export err=$?;err_chk
...
if [ $CYCNEST = true ]; then
  mv input_domain_01_nemsio $GESDIR/nam.t${cyc}z.input_domain_${domain}nest_nemsio.tm06_precoldstart
  cpreq $GESDIR/nam.t${cyc}z.input_domain_${domain}nest_nemsio.tm06_precoldstart  $GESDIR/nam.t${cyc}z.input_domain_${domain}nest_nemsio.tm06
  export err=$?;err_chk
else
