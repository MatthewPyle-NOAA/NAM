       SUBROUTINE  PRECIP48H( JCYCLE, K, IIP, JJP, FLDC )
C$$$  SUBPROGRAM DOCUMENTATION BLOCK                                  
C                                                                     
C SUBPROGRAM:  PRECIP48H      COMPUTES 6 HOURLY PRECIP AMOUNTS
...
C                                                                     
C USAGE:    CALL PRECIP48H( FILE, IDTBL, IHOUR, K )                      
...
C
     ...
     A      K .EQ. 11 .OR. K .EQ. 15 ) THEN
C        SET FLAG TO INDICATE THE 6, 18, 30, AND 42-HOURLY
C        PRECIPITATION BUCKETS
         IPREC = 0
       ELSE IF ( K .EQ. 5 .OR. K .EQ. 9 .OR.
     A      K .EQ. 13 .OR. K .EQ. 17 ) THEN
C        SET FLAG TO INDICATE THE 12, 24, 36, AND 48-HOURLY
C        PRECIPITATION BUCKETS
         IPREC = 1
       ELSE IF ( IREM .EQ. 0 ) THEN
C        SET FLAG TO INDICATE THE 3, 9, 15, 21, 27, 33, 39 AND 
C        45-HOURLY PRECIPITATION BUCKETS
         IPREC = 2
       ELSE
         RETURN
       ENDIF
