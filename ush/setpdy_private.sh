...
if [ -z "$PDY" ]; then
   PDY=$(cut -c7-14 ${COMDATEROOT}/date/$cycle)
   cp ${COMDATEROOT}/date/$cycle ncepdate
else
   sed "s/[0-9]\{8\}/$PDY/" ${COMDATEROOT}/date/$cycle > ncepdate
fi
...
if [ -f PDY.${jobid:?} ]; then rm PDY.$jobid ; fi
for d in $(seq $dates_before_PDY -1 1); do
   # cut the date starting at position ($d-1)*9 from date-line
   echo "export PDYm${d}=${DATElne_minus:((($d-1)*9)):8}" >> PDY.$jobid
done
echo "export PDY=$PDY" >> PDY.$jobid
for d in $(seq $dates_after_PDY); do
   # cut the date starting at position ($d-1)*9 from date-line
   echo "export PDYp${d}=${DATElne_plus:((($d-1)*9)):8}" >> PDY.$jobid
done
