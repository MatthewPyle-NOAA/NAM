...
export hswitch=$2
export PS4='PRDGEN7_F${fhr}_T$SECONDS + '
export compress_type=c3

cd $DATA/prdgen7_${fhr}
...
if [ $gridnum -eq 181 ] ; then
  filnam=afwaca
  gridspecs="latlon 260.0:370:0.108 0.138:278:0.108"
fi
if [ $gridnum -eq 183 ] ; then
  filnam=afwaak
  gridspecs="latlon 160.0:648:0.108 45.138:278:0.108"
fi
...
then
    ...
    then
     ...
     if [ $filename = afwaca ] ; then
        $WGRIB2 -s $COMOUT/${RUN}.${cycle}.${filename}${fhr}.${tmmark}.grib2 > $COMOUT/${RUN}.${cycle}.${filename}${fhr}.${tmmark}.grib2.idx
        $DBNROOT/bin/dbn_alert MODEL NAM_${REGION}_GB2_WIDX $job $COMOUT/${RUN}.${cycle}.${filename}${fhr}.${tmmark}.grib2.idx
     fi
    fi
fi
...
if test "$SENDECF" = 'YES'
then
  ecflow_client --event post_7_complete
fi
