...
!-----------------------------------------------------------------------
     ...
     &                      its,ite, jts,jte, kts,kte,d_ss,mprates,     & !jul28
     &                      refl_10cm )              !jul28
!-----------------------------------------------------------------------
      IMPLICIT NONE
!-----------------------------------------------------------------------
     ...
     &                      qc,qr,qs
      REAL, INTENT(INOUT),  DIMENSION(ims:ime, jms:jme,kms:kme) ::    &
     &                      F_ICE_PHY,F_RAIN_PHY,F_RIMEF_PHY
      REAL, INTENT(OUT),    DIMENSION(ims:ime, jms:jme,kms:kme) ::      & !jul28
     &                      refl_10cm               !jul28
...
!     LOCAL VARIABLES
!-----------------------------------------------------------------------
      REAL :: BBFR,DTPH,DX,Thour_print,RDIS,BETA6
...
!
        IF(MYPE==0)THEN
          ...
          IF (etampnew_unit1<0) THEN
            ...
            write(0,*)' ABORTING!'
            CALL ESMF_FINALIZE(endflag=ESMF_END_ABORT,rc=IRTN)
          ENDIF
        ENDIF
!
