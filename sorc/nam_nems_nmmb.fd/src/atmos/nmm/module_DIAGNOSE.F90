...
!!!!!!!! REFD @ - 10 C !!!!!!!!!!!!!!!!!!!!
      ...
      DO J=JTS_B1,JTE_B1
       DO I=ITS_B1,ITE_B1
        DBZ1(1)=DBZmin
        DBZ1(2)=DBZmin
  vloopm10: DO L=LM,2,-1  ! search from ground up
          IF ( (T(I,J,L)   .GE. 263.15)                &
          .AND.(T(I,J,L-1) .LE. 263.15) )  THEN
            DBZ1(1)=REFL_10CM(I,J,L)   !- dBZ (not Z) values
            DBZ1(2)=REFL_10CM(I,J,L-1) !- dBZ values
            EXIT vloopm10
          ENDIF
...
!
         DO LL=1,2
           ...
           DBZ1(LL)=CUREFL+REFL    !- in Z units
         ENDDO
!-- Pure mean of the bounding reflectivity values
!        DBZ1avg=0.5*(DBZ1(1)+DBZ1(2))
         DBZ1avg=maxval(DBZ1)
!-- Convert to dBZ (10*logZ) as the last step
         IF (DBZ1avg>ZMIN) THEN
           DBZ1avg=10.*ALOG10(DBZ1avg)
         ELSE
           DBZ1avg=DBZmin
         ENDIF
         REFDMAX263K(I,J)=max(REFDMAX263K(I,J),DBZ1avg)
       ENDDO
      ENDDO
