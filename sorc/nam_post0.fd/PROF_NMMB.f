...
C   10-04-26  ROGERS - CHANGED TIMESTAMP FROM HOURS to MINUTES
C
C USAGE:  CALL PROF FROM PROGRAM POST0
C
C   INPUT ARGUMENT LIST:
C     FILENAME - HISTORY FILE NAME
C     LRSTRT - THE UNIT NUMBER FOR READING THE RESTRT FILE
C     ITAG   - ELAPSED FORECAST TIME IN MINUTES OF THE HISTORY FILE WE ARE DEALING WITH
C     INCR   - FORECAST INTERVAL BETWEEN HISTORY FILES IN MINUTES
C
...
C-----------------------------------------------------------------------
                             P A R A M E T E R
     & (NSTAT=2000,LCL1ML=15,LCL1SL=52
     &, D608=0.608)
C-----------------------------------------------------------------------
C
...
C	new stuff
      ...
      character(len=256) :: fileName_prev
...
!      Get nrec before allocating arrays
	...
	me=0
      ...
      VarName='TPH0D'
        ...
        if (iret /= 0) then
         ...
         TPH0D=garb
         write(6,*) "cenlat, tph0d ",cenlat, tph0d
        end if
      ...
      VarName='TLM0D'
        ...
        if (iret /= 0) then
         ...
         TLM0D=garb
         write(6,*) "cenlon, tlm0d ",cenlon, tlm0d
        end if
...
!	TPH0D=cenlat/1000.
        ...
        print *,"cenlat,cenlon,tlm0d,tph0d ",cenlat,cenlon,tlm0d,tph0d
...
!       write(0,*) 'L, PINT(N,L): ',  N, L, PINT(N,L), PMID(N,L)
        endif
...
c
	...
	write(6,*) 'IFHR: ', IFHR,'IFMIN: ',IFMIN
C------------------------------------------------------------------------
c     IF(IFHR.GT.0.AND.IFMIN.GT.0)THEN
      IF(ITAG.GT.0)THEN
C***
...
C***
      ITAGPREV=ITAG-INCR
        ...
        if (MOD(ITAGPREV,60).EQ.0) THEN
          IFHRPREV=ITAGPREV/60
          IFMINPREV=0
        else
          IFHRPREV=ITAGPREV/60
          IFMINPREV=MOD(ITAGPREV,60)
        endif
...
!        write(DateStrold,301) JDATE(1),JDATE(2),JDATE(3),JDATE(5)
 301    format(i4,'-',i2.2,'-',i2.2,'_',i2.2,':00:00')
        ...
        write(ITAGLAB,302) IFHRPREV
 302	format(I3.3)
        write(IMINLAB,303) IFMINPREV
 303	format(I2.2)

	write(6,*) 'filename later in PROF: ', filename, '_END'
        len=lnblnk(filename)
	write(6,*) 'LEN= ', LEN
c       write(6,*) 'carried over part: ', filename(1:len-3)
        write(6,*) 'carried over part: ', filename(1:len-15)
...
C***
C***  OPEN/READ THE PREVIOUS RESTRT FILE
C***
...
!-------------------------------------------------------------------
      ...
      l=1
      call getnemsandplace(nfile,im,jm,spval,VarName,VcoordName,
     &               l,impf,jmpf,nframed2,NUMSTA,IHINDX,JHINDX,AVRAIN0)
...
C  12km NAM
	elseif (DT .ge. 25 .and. DT .le. 28) then 
        ...
        NCNVC=6
C  6 km Alaska nest
	elseif (DT .ge. 8.9 .and. DT .le. 13.4) then 
        NPHS=5
        NCNVC=5
C  4 km CONUS nest
        elseif (DT .lt. 8.9) then
        NPHS=5
        NCNVC=5
C  3 km Hawaii/Puerto Rico nest
        elseif (DT .lt. 7) then
        NPHS=9
        NCNVC=9
        endif
...
c       IFCST=3600*ITAG
c  itag now in minutes          
        IFCST=60*ITAG       
	TIME=IFCST
	NTSD=(TIME/DT)+1

	write(6,*) 'TIME, DT, NTSD: ', TIME, DT, NTSD, NPHS

      RESET0=TIME-(NTSD/NPREC)*NPREC*DT
c     RESET1=(NPHS-1)*DT+3600.
c   incr no longer assumed to be hardwired to 3600 sec (1 hour)
      RESET1=(NPHS-1)*DT+(INCR*60.)
C
...
C
      RESET0=TIME-(NTSD/NHEAT)*NHEAT*DT
        ...
        write(6,*) 'RESET0, RESET1 for HEAT: ', RESET0, RESET1
        write(6,*) 'NTSD, NHEAT, NCNVC: ', NTSD, NHEAT, NCNVC
        write(6,*) 'RESET0 < RESET1 for heating? : ', RESET0, RESET1
        write(6,*) 'MOD < NCNVC for heating? : ', mod(NTSD,NHEAT),NCNVC
...
C
       IFCST=60*ITAG
	write(6,*) 'IHR, IMIN: ', IHR, IMIN
C
      WRITE(6,*)' POST PROFILE FOR ',
     1                       IYR,IMNTH,IDAY,IHR,IMIN
      write(6,*) 'IHRST= ', IHRST
C
...
C
      ...
      ELSE
        RTSPH=1.
      ENDIF
	write(6,*) 'APHTIM, RTSPH: ',APHTIM(1),APHTIM(1)-APHTIM0(1),RTSPH
C
...
C
	write(0,*) 'trial d'
      ...
      ELSE
        ...
        RTSRA=1.
      ENDIF
	write(6,*) 'ACUTIM, RTSCU: ',ACUTIM(1),ACUTIM(1)-ACUTIM0(1),RTSCU
C
c     IF(AVRAIN(N).GT.0.)THEN
c       RTSRA=1./(AVRAIN(N)-AVRAIN0(N))
c     ELSE
c       RTSRA=1.
c     ENDIF
c      write(6,*) 'AVRAIN, RTSRA: ',AVRAIN(N),AVRAIN(N)-AVRAIN0(N), RTSRA
C
C--------------------------------------------------------------------------
C--------------------------------------------------------------------------
C***
...
!      NREC2=(IFHR/INCR)*NUMSTA+N
      NREC2=N
!      write(6,*) 'NREC2,IHRST,IDAT: ', NREC2,IHRST,IDAT,IFCST

!        if (mod(NREC2,10) .eq. 0) then
!        write(6,*) 'IDAT, IFCST, ISTAT, CISTAT: ',
!     &  IDAT, IFCST, ISTAT, CISTAT
!        endif
!
      WRITE(LCLAS1,REC=NREC2)IHRST,IDAT,IFCST,ISTAT,CISTAT
