...
C      SUBROUTINE PROF(ITAG,LCLAS1)
      SUBROUTINE PROF_NMM(filename,startdate,ITAG,INCR)
C
...
! former parameter statements
     ...
     &,DUM3D(IM,LM,JM),DUM3D2(IM,LM+1,JM)
     &,DUM3D3(IM,LM+1,JM),DUMMY2(IM,JM),DUMMY(IM,JM)
...
!          HBOT(N)=DUMMY2(I,J)
     ...
     + , mpi_status_ignore, ierr)
	    ...
	    I=IHINDX(N)	
	    J=JHINDX(N)
             PINT (N,L) = dum3d2 (I,LL,J) 

	if (N .eq. 5) then
	write(6,*) 'N,L,PINT(N,L): ', N,L,PINT(N,L)
	endif

c            if (L .ge. 2) then
c             PMID ( N,L-1 ) =  EXP( 0.5* (ALOG(DUM3D2(I,LL-1,J))+
c    &                                     ALOG(DUM3D2(I,LL,J))) )
c              if(n.eq.10) then
...
c            endif
        ...
        DO L=2,LP1
         DO N=1,NUMSTA
           ...
           if (n.eq.10) then
             write(6,*) 'N,L,PMID,PINTs: ',n,l,PMID(N,L-1),
     &                             PINT(N,L),PINT(N,L-1)
           endif
         end do
        end do
...
!        PRODAT(LVL)      = PDSL1(N)*AETA(LV)+PT
	...
	write(6,*) 'N, LV, PMID:: ', N, LV, PMID(N,LV)
