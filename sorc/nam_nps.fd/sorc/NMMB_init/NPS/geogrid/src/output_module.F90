...
#include "wrf_status_codes.h"
      ...
      integer :: sp1, ep1, sp2, ep2, ep1_stag, ep2_stag, N
      real :: dx, dy, cen_lat, cen_lon, moad_cen_lat, dx_tmp(20), dy_tmp(20)
...
#ifdef IO_NETCDF
      if (io_form_output == NETCDF_FORMAT) call ext_ncd_ioinit('sysdep info', istatus)
#endif
#ifdef IO_DIO
      if (io_form_output == DIO_FORMAT) call dio_init(iret=istatus)
#endif
...
#ifdef IO_NETCDF
      if (io_form_output == NETCDF_FORMAT) call ext_ncd_inquiry('OPEN_COMMIT_WRITE',coption,istatus)
      ...
      istatus = 0
#ifdef IO_NETCDF
      if (io_form_output == NETCDF_FORMAT) call ext_ncd_inquiry('SUPPORT_3D_FIELDS',coption,istatus)
#endif
#ifdef IO_DIO
      if (io_form_output == DIO_FORMAT) coption = 'YES'
#endif
      call mprintf((istatus /= 0), ERROR, 'Error in ext_pkg_inquiry')
...
! BUG: What if we have no plans to write 3-d fields? We should take this into account.
      ...
      if (io_form_output == NETCDF_FORMAT) suffix='nc'
      if (io_form_output ==    DIO_FORMAT) suffix='dio'

#ifdef _GEOGRID
      output_fname = ' '
      if (grid_type == 'C') then
         output_fname = trim(opt_output_from_geogrid_path)//'geo_em.d  .'//trim(suffix)
         ...
         if (nest_number == 1) then
            output_fname = trim(opt_output_from_geogrid_path)//'geo_nmm.d  .'//trim(suffix)
            i = len_trim(opt_output_from_geogrid_path)
            write(output_fname(i+10:i+11),'(i2.2)') nest_number
         else
            output_fname = trim(opt_output_from_geogrid_path)//'geo_nmm_nest.l  .'//trim(suffix)
            i = len_trim(opt_output_from_geogrid_path)
            write(output_fname(i+15:i+16),'(i2.2)') nest_number-1
         end if
      else if (grid_type == 'B') then
            output_fname = trim(opt_output_from_geogrid_path)//'geo_nmb.d  .'//trim(suffix)
            i = len_trim(opt_output_from_geogrid_path)
            write(output_fname(i+10:i+11),'(i2.2)') nest_number
      else if (grid_type == 'A') then
            output_fname = trim(opt_output_from_geogrid_path)//'geo_sla.d  .'//trim(suffix)
            i = len_trim(opt_output_from_geogrid_path)
            write(output_fname(i+10:i+11),'(i2.2)') nest_number
      end if
      ...
      output_fname = ' '
      if (grid_type == 'C') then
         output_fname = trim(opt_output_from_metgrid_path)//'met_em.d  .'//trim(datestr)//'.'//trim(suffix)
         i = len_trim(opt_output_from_metgrid_path)
         write(output_fname(i+9:i+10),'(i2.2)') nest_number
      else if (grid_type == 'E') then
         output_fname = trim(opt_output_from_metgrid_path)//'met_nmm.d  .'//trim(datestr)//'.'//trim(suffix)
         i = len_trim(opt_output_from_metgrid_path)
         write(output_fname(i+10:i+11),'(i2.2)') nest_number
      else if (grid_type == 'B') then
         output_fname = trim(opt_output_from_metgrid_path)//'met_nmb.d  .'//trim(datestr)//'.'//trim(suffix)
         i = len_trim(opt_output_from_metgrid_path)
         write(output_fname(i+10:i+11),'(i2.2)') nest_number
      else if (grid_type == 'A') then
         output_fname = trim(opt_output_from_metgrid_path)//'met_sla.d  .'//trim(datestr)//'.'//trim(suffix)
         i = len_trim(opt_output_from_metgrid_path)
         write(output_fname(i+10:i+11),'(i2.2)') nest_number
      end if
      ...
      if (supports_training) then
         ...
         if (my_proc_id == IO_NODE .or. do_tiled_output) then
            istatus = 0
#ifdef IO_NETCDF
            if (io_form_output == NETCDF_FORMAT) then
         ...
         if (my_proc_id == IO_NODE .or. do_tiled_output) then
            istatus = 0
#ifdef IO_NETCDF
            if (io_form_output == NETCDF_FORMAT) call ext_ncd_open_for_write_commit(handle, istatus)
#endif
         ...
         if (my_proc_id == IO_NODE .or. do_tiled_output) then
            ...
            if (io_form_output == NETCDF_FORMAT) then
               ...
               write(0,*) trim(output_fname),istatus
            end if
#endif
#ifdef IO_DIO
            if (io_form_output == DIO_FORMAT) then
               call dio_open(dfile,trim(output_fname),"WRITE",iret=istatus)
            end if
#endif
            call mprintf((istatus /= 0),ERROR,'Error in ext_pkg_open_for_write')
         end if
      ...
      else if (grid_type == 'E' .or. grid_type =='B' .or. grid_type == 'A') then
         ep1 = ep1
         ep2 = ep2
         ep1_stag = ep1
         ep2_stag = ep2
      ...
      if (grid_type == 'C' .or. grid_type == 'A') then
	...
	dy_tmp(1)=dykm
	DO N=2,nest_number
	    dx_tmp(N)=dx_tmp(parent_id(N))/(parent_grid_ratio(N))
	    dy_tmp(N)=dy_tmp(parent_id(N))/(parent_grid_ratio(N))
        ENDDO
	print*, 'writing dx, dy: ', dx_tmp(nest_number), dy_tmp(nest_number)
         dx = dx_tmp(nest_number)    ! For NMMB, need to worry about parent and ratios
         dy = dy_tmp(nest_number)
         moad_cen_lat = 0.
         cen_lat=known_lat
         cen_lon=known_lon
      end if
      ...
      call write_global_attrs(title, datestr, grid_type, dynopt, ixdim(nest_number), jydim(nest_number), 0, &
                              ...
                              proj_stack(nest_number)%i_parent_start,proj_stack(nest_number)%j_parent_start, &
                              proj_stack(nest_number)%i_parent_end  ,proj_stack(nest_number)%j_parent_end,   &
   ...
   subroutine init_output_fields(grid_type, &
      ...
      if (grid_type == 'E' .or. grid_type == 'B') NUM_AUTOMATIC_FIELDS = 5
      if (grid_type == 'A') NUM_AUTOMATIC_FIELDS = 4
      ...
      if (grid_type == 'C') then
      ...
      else if (grid_type == 'E' .or. grid_type == 'B') then
         fields(1)%fieldname = 'XLAT_M'
         fields(1)%units = 'degrees latitude'
         fields(1)%descr = 'Latitude on mass grid'
         ...
         fields(5)%fieldname = 'LANDMASK'
         fields(5)%units = 'none'
         fields(5)%descr = 'Landmask : 1=land, 0=water'
         ...
         fields(3)%fieldname = 'MAPFAC_M'
         fields(3)%units = 'none'
         fields(3)%descr = 'Mapfactor on mass grid'

         fields(4)%fieldname = 'LANDMASK'
         fields(4)%units = 'none'
         fields(4)%descr = 'Landmask : 1=land, 0=water'
...
!      for staggering if it is alright to pass true domain dimensions to write_field.
               ...
               if (io_form_output == DIO_FORMAT) then
               header=0
               call wrfheader_pack(header, DateStr=datestr, &
                                           VarName=trim(fields(i)%fieldname), &
                                           Units=trim(fields(i)%units), &
                                           Description=trim(fields(i)%descr), &
                                           FieldType=DIO_REAL4, &
                                           MemoryOrder=trim(fields(i)%mem_order), &
                                           Stagger=trim(fields(i)%stagger), &
                                           DimNames=fields(i)%dimnames, &
                                           DomainStart=sd, DomainEnd=ed, &
                                           PatchStart=sp, PatchEnd=ep &
                                  )

               call dio_write(dfile,trim(fields(i)%fieldname), &
                              real_dom_array(sd(1):ed(1),sd(2):ed(2),sd(3):ed(3)), &
                              header=header, iret=istatus)
               end if
#endif
...
!                                                 trim(fields(i)%fieldname),(/fields(i)%sr_y/),1, istatus)
...
#endif
#ifdef IO_DIO
      if (io_form_output == DIO_FORMAT) call dio_write(dfile,trim(var_name),var_value,iret=istatus)
#endif
...
#endif
#ifdef IO_DIO
      if (io_form_output == DIO_FORMAT) call dio_write(dfile,trim(var_name),var_value(1:n),iret=istatus)
#endif
      call mprintf((istatus /= 0),ERROR,'Error in writing domain time-independent attribute')
   ...
   subroutine output_close()
      ...
      if (my_proc_id == IO_NODE .or. do_tiled_output) then
         ...
         istatus = 0
#ifdef IO_NETCDF
         if (io_form_output == NETCDF_FORMAT) call ext_ncd_ioclose(handle, istatus)
#endif
#ifdef IO_DIO
         if (io_form_output == DIO_FORMAT) call dio_close(dfile,iret=istatus)
#endif
         call mprintf((istatus /= 0), ERROR, 'Error in ext_pkg_ioclose')
   
         istatus = 0
#ifdef IO_NETCDF
         if (io_form_output == NETCDF_FORMAT) call ext_ncd_ioexit(istatus)
#endif
#ifdef IO_DIO
         if (io_form_output == DIO_FORMAT) call dio_finalize()
#endif
